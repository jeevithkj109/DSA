...
public class RecursiveOptimized {
	...
	private static int findMinCost(int start, int end, List<Integer> cuts) {
		...
		for(int k = start+1; k < end; k++ ) {
			...
			int answer = cuts.get(end)-cuts.get(start) + findMinCost(start, k, cuts) + findMinCost(k, end, cuts); 
	...
	public static void main(String[] args) {
		...
		System.out.println(findMinCost(0, cutsList.size()-1, cutsList));
	}
}
