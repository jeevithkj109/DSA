...
public class SpaceOptimized {
	...
	private static int maximumProfit(int transaction, int[] prices) {
		...
		int[][] prev = new int[3][transaction+1];
		//Initialization
		// for k == 0: make all values 0 (default)
		// if j == 1 make values to int_min
		for(int k = 0; k <= transaction; k++) {
			prev[1][k] = Integer.MIN_VALUE;
		}
		
		for(int i = n-1; i >= 0; i--) {
			
			int[][] curr = new int[3][transaction+1];

			for(int j = 0; j <= 2; j++) {
				for(int k = 1; k <= transaction; k++) {
					if(j == 2) {
						curr[j][k] = Math.max(prev[2][k], Math.max(-prices[i]+prev[0][k], prices[i]+prev[1][k]));
					}
					else if(j == 1) {
						curr[j][k] = Math.max(-prices[i]+prev[2][k-1], prev[1][k]);
					}
					else {
						curr[j][k] =Math.max(prices[i]+prev[2][k-1], prev[0][k]);
					}
				}
			}
			prev = curr;
